...
- [~] **1e. Editor** — parts 1+2 DONE (2026-07-12): /app unified with full brand UI
  (mark, titles, folder groups, daily capture) + CodeMirror 6 markdown editing
  pushing encrypted updates; daemons auto-sync every 30s (src/autosync.ts);
  concurrent cross-device edits now 3-WAY MERGE (diff3 vs .vortex/sync-base/),
  conflict files only on overlapping hunks. DECISION: Yjs CRDT deferred until the
  real-time collab channel exists (WebSocket push) — diff3 ≈ CRDT at 30s cadence,
  and avoids the double-bootstrap migration hazard. Part 3 TODO: TipTap block
  view, bundle diet (711KB), WS push channel.
